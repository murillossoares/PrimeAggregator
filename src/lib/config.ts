...
const PairSchema = z.object({
  ...
  minNotionalA: z.string().regex(/^\d+$/).optional(),
  ...
  cooldownOnLossMs: z.number().int().min(0).optional(),
  maxTradesPerHour: z.number().int().min(0).optional(),
  maxDailyLossA: z.string().regex(/^\d+$/).optional(),
  maxNotionalA: z.string().regex(/^\d+$/).optional(),
  maxPriceImpactBps: z.number().int().min(0).max(10_000).optional(),
  maxRouteHops: z.number().int().min(1).max(64).optional(),
